def bicycle(arr, cost, left, right):
    if right <= left:
        return -1
    if arr[left] >= cost:
        return left + 1
    mid = (left + right) // 2
    if arr[mid - 1] < cost <= arr[mid]:
        return mid + 1
    elif cost <= arr[mid]:
        return bicycle(arr, cost, left, mid)
    else:
        return bicycle(arr, cost, mid + 1, right)


if __name__ == '__main__':
    _right = int(input())
    _arr = [int(i) for i in input().split()]
    _cost = int(input())
    print(
        bicycle(_arr, _cost, 0, _right),
        bicycle(_arr, _cost * 2, 0, _right)
    )
